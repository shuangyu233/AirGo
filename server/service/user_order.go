...
func (o *Order) GetUserTotalConsumptionAmount(uID int64) (float64, error) {
	...
	err := global.DB.
		Model(&model.Order{}).
		Where("user_id = ? AND pay_type in ? AND trade_status in ?", uID, []string{constant.PAY_TYPE_ALIPAY, constant.PAY_TYPE_EPAY}, []string{constant.ORDER_STATUS_TRADE_SUCCESS}).
		Select("total_amount").
		Find(&list).Error
	if err != nil {
		return 0, err
